...
if (status_area) {
  const hashtag_id = status_area.dataset.id;

  const interval = setInterval(() => {
    fetch(`/hashtag-status/${hashtag_id}`)
      .then((res) => res.json())
      .then((data) => {
          ...
          status_area.innerHTML = `
                    <div class="mt-12 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                        ...
                        <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-indigo-50 to-purple-50 border-b">
                            <div>
                                <h2 class="text-xl font-semibold text-gray-800">Your Generated Hashtags</h2>
                            </div>
                        </div>
                        ...
                        <div class="p-8">
                            <article class="not-prose">
                                ...
                                <div class="flex flex-wrap gap-3">
                                    ${data.response.map(hashtag => `
                                        <div class="group flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-600 to-pink-400 text-white text-sm font-medium rounded-full shadow-md hover:scale-105 transition cursor-pointer">
                                            <span>${hashtag}</span>
                                        </div>
                                    `).join("")}
                                </div>
